...
> **info** Pull requests opened from a fork of the connected repository do not trigger `pull_request` workflow runs.
...
| `corepack`  | If set to `true`, [corepack](https://github.com/nodejs/corepack#readme) will be enabled at the beginning of build process. Defaults to false. |
...
- A missing value renders as an empty string, so every run without that value gets the same key and shares one group. For example, `github.ref` is empty when you start a run with `eas workflow:run` from your local project without `--ref`, so `deploy-${{ github.ref }}` puts all such runs in the group `deploy-`. Use `||` to provide a fallback: `deploy-${{ github.ref || 'manual' }}`.
...
concurrency:
  group: e2e-${{ github.event.pull_request.number }}
  # @info #
  cancel_in_progress: ${{ github.event.action != 'edited' }}
  # @end #
```
...
**Cancel stale runs of a branch.** Each new run cancels the active run and the waiting runs of this workflow for the same branch. Only the newest run remains.
...
**Never interrupt a deploy, and never queue more than one run.** The active deploy always finishes. The newest run waits for it. EAS replaces the runs in between, and they never start.
...
**Run everything in order.** Runs start one at a time, in creation order. Up to 100 runs can wait.
...
**Cancel on new commits, but not on pull request edits.** A run for a new commit cancels the active run. A run for an edit to the pull request's title, body, or base branch waits for the active run instead.
...
**Share one group across workflow files.** Runs of both workflows share one group and run one at a time.

```yaml
# In both deploy-ios.yml and deploy-android.yml
concurrency:
...
| [`app_store_connect`](#app_store_connect)                                                               | <YesIcon />  | <YesIcon /> | `build` jobs only |
...
```yaml
run_name: Deploy ${{ app.slug }} from ${{ github.ref_name }}
```
...
jobs:
  notify_slack:
    type: slack
    params:
      message: ${{ account.name }}/${{ app.slug }} finished ${{ workflow.name }}
```
  ...
  test:
    steps:
      ...
      - name: Upload results
        ...
        if: ${{ always() }}
        # @end #
        run: echo "Runs even when the tests failed"
```
...
Submits an Android or iOS build to the app store using [EAS Submit](/deploy/submit-to-app-stores/). See [Submit job documentation](/eas/workflows/pre-packaged-jobs#submit) for detailed information and examples.
...
```json
{
  "apple_app_id": string | null, // Apple App ID. https://expo.fyi/asc-app-id
  "ios_bundle_identifier": string | null, // iOS bundle identifier of the submitted build. https://expo.fyi/bundle-identifier
  "asc_build_id": string | null // App Store Connect build ID. Only when submitting an already uploaded build.
}
```
...
Runs [Maestro](https://maestro.dev/) tests on a build in [Maestro Cloud](https://docs.maestro.dev/maestro-cloud/run-tests-on-maestro-cloud). See [Maestro Cloud job documentation](/eas/workflows/pre-packaged-jobs#maestro-cloud) for detailed information and examples.

> **important** Running tests in Maestro Cloud requires a Maestro Cloud account and Cloud Plan subscription. Go to [Maestro docs](https://docs.maestro.dev/maestro-cloud/run-tests-on-maestro-cloud) to learn more.
