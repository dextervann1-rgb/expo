...
import { groupScannedModules, resolveScannerPlugin, scanExpoModulesAsync } from './moduleScanner';
...
export async function scanNativeModulesAsync(
  ...
  for (const [packageName, revision] of Object.entries(searchResults)) {
    packageRoots[packageName] = revision.path;
  }
...
/**
 * The modules to put into the generated provider: the ones declared in `expo-module.config.json`
 * when the package declares any (an explicit declaration is a full override, so a class the config
 * deliberately leaves out is never linked), otherwise the scanned `@ExpoModule` classes, in a
 * stable alphabetical order.
 */
function pickModules(
  configModules: ModuleIosConfig[],
...
export async function resolveModuleAsync(
  ...
  return {
    ...
    modules: pickModules(configModules, extraOutput.scannedModules?.[packageName] ?? []),
